...
class SQ4Quantizer : public QuantizerBase<float, uint8_t> {
 public:
  ...
  void reorder(const Pool& pool, int* dst, int k) const {
    if (reorder_quantizer_) {
      int cap = pool.capacity();
      searcher::MaxHeap<float> heap(k);
      for (int i = 0; i < cap; ++i) {
        if (i + 1 < cap) {
          reorder_quantizer_->prefetch_data(pool.id(i + 1), 1);
        }
        int id = pool.id(i);
        float dist = reorder_quantizer_->compute_query_distance(id);
        heap.push(id, dist);
      }
      for (int i = 0; i < k; ++i) {
        dst[i] = heap.pop();
 ...
 private:
  ...
  std::unique_ptr<FP32Quantizer> reorder_quantizer_;  // 用于精排的FP32量化器
